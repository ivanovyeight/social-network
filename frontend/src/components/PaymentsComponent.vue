<template>
  ...
  <v-card class="mb-2">
    ...
    <v-container>
      <v-row>
        <v-col cols="12" md="6" class="col-md-6 text-center" v-for="(card, index) in paymentMethods" :key="card.id">
          <v-card outlined>
            <v-btn text disabled>
              {{
  ...
  <v-btn block class="mb-2" @click="getSubscriptionPlans()">Subscribe To Premium</v-btn>

  <v-stepper v-model="subscriptionStep" v-if="subscribeToPremiumModeActive == true">
    <v-stepper-header>
...
export default {
  ...
  methods: {
    ...
    addPaymentMethod() {
      http.get("http://localhost:8000/payments/payment-methods/setup-token/")
        .then(response => {
    ...
    getSubscriptionPlans() {
      ...
      http.get("http://localhost:8000/payments/subscriptions/")
